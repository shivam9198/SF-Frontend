...
function KpiCard({ metric }) {
    ...
    return (
        <Card className="group flex flex-col justify-between overflow-hidden transition hover:-translate-y-0.5 hover:shadow-xl !p-4 sm:!p-5">
            <div className="flex items-start justify-between gap-3 sm:gap-4">
                <div className="min-w-0 flex-1">
                    <p className="text-xs sm:text-sm font-medium text-slate-500 dark:text-slate-400 truncate">{metric.label}</p>
                    <p className="mt-1 sm:mt-2 text-lg sm:text-xl lg:text-2xl font-bold text-slate-900 dark:text-white truncate" title={metric.key === 'collection' ? formatCurrency(metric.value) : metric.key === 'rate' ? `${metric.value}%` : metric.value}>
                        {metric.key === 'collection' ? formatCurrency(metric.value) : metric.key === 'rate' ? `${metric.value}%` : metric.value}
                    </p>
                </div>
